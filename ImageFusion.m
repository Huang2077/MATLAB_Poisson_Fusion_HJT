...
clc;
%载入图像
TargetImg = imread('融合用图/poolRGB.jpg');
SourceImg = imread('融合用图/bearRGB.jpg');
SourceMask = im2bw(imread('融合用图/bearMask.jpg'));
%获取裁剪边界
...
VNew = cat(3, VR, VG, VB);
figure, imagesc(uint8(VNew)), axis image
...
figure, imagesc(uint8(ImgFusion)), axis image, title('FusionImage');
